...
public class TestHttpRequest
{
	 ...
	 */
	public static final String HTTP_CHARACTER_ENCODING_HEADER = "Character-encoding";
	...
	static
	{
		...
		catch (MalformedURLException mE)
		{
			try
			{
				Class msFactory = Class
						.forName("com.ms.net.wininet.WininetStreamHandlerFactory");
				URL
						.setURLStreamHandlerFactory((URLStreamHandlerFactory) msFactory
								.newInstance());
				System.err
						.println("Using stream handler from com.ms.net.wininet");
			}
			catch (Throwable t)
			{
				try
				{
					...
					Class securityClass = Class
							.forName("java.security.Security");
					java.lang.reflect.Method addProviderMethod = securityClass
							.getMethod("addProvider", new Class[]
							{ Class.forName("java.security.Provider") });
					Class sslProviderClass = Class
							.forName("com.sun.net.ssl.internal.ssl.Provider");
					addProviderMethod.invoke(null /* static method */
					, new Object[]
					{ sslProviderClass.newInstance() });
				...
				catch (Throwable t2)
				{
					t2.printStackTrace();
					System.err
							.println("Neither Microsoft nor JSSE SSL support available.");
					System.err
							.println("In fact, there is no HTTPS support at all.");
					System.err
							.println("Expect exceptions on all connection attempts!");
				}
			}
		}
	}
	 ...
	 *            the URL to connect
	 */
	public TestHttpRequest(String url) throws MalformedURLException
	{
	 ...
	 *            the URL to connect
	 * @param post
	 *            set to true to to an HTTP post
	 * @param contentType
	 *            set the content type for an http post
	 */
	public TestHttpRequest(String url, boolean post, String contentType)
	...
	public void parseStatusCode(URLConnection urlConnection)
	{
		...
		try
		{
			// First, try to get the HTTP/1.1 response header Mac IE and Windows
			// IE style
			headers = urlConnection.getHeaderField(0);

			// If this fails, try this way that works on Windows IE.
			// It'll crash Mac IE, but Mac IE would've worked on the previous
			// try.
			// This one is probably unnecessary. I'm pretty sure that anything
			// that works
			// with this would work with the one just above it.
			// 
			// There's an exception handler because it throws a
			// NullPointerException in Opera
			try
			{
				if (headers == null)
				{
			...
			// Sun VM 1.3 only has content type and content length headers
			// available, both based
			// on a "best guess algorithm" (see
			// http://java.sun.com/products/plugin/1.3/docs/https.html).
			// All we can do is assume HTTP_OK if no other HTTP response codes
			// are found in the
			...
			{
				String statusString = headers
						.substring(headers.indexOf(" ") + 1);
				spaceIndex = statusString.indexOf(" ");
				if (spaceIndex != -1 && spaceIndex != statusString.length() - 1)
				{
					try
					{
						statusCode = Integer.parseInt(statusString.substring(0,
								spaceIndex));
				...
				try
				{
					...
					if (post)
					{
						...
						urlConnection.setRequestProperty(
								HTTP_CONTENT_TYPE_HEADER, outgoingContentType);
			...
			if (X_WWW_FORM_URL_ENCODED.equals(outgoingContentType))
			{
				...
				while (e.hasMoreElements())
				{
					...
					if (e.hasMoreElements())
						getOutputStream()
								.write("&".getBytes(characterEncoding));
				}
			}
			else if (MULTIPART_FORM_DATA.equals(outgoingContentType.substring(
					0, MULTIPART_FORM_DATA.length())))
			{
				while (e.hasMoreElements())
				{
					key = (String) e.nextElement();
	...
	public void setForm(Hashtable form)
	{
		...
		if (!post)
			throw new IllegalStateException(
					"Can't specify a form for an HTTP GET request");
		this.form = form;
	}
	...
	private URLConnection debugSSL(URL url) throws IOException
	{
		try
		{
			...
			// with one of our own since we want the client to bypass the peer
			// and
			// submitted host checks even
